...
## Digital Medication Administration Record — MAR (completed 2026-09-02)
Converts the paper MAR (per-medication, per-scheduled-time, per-day grid, initialed by whoever gave the dose) into a real feature layered on top of the existing `PatientMedication` prescription record — `PatientMedication` stays metadata-only; a new administration log tracks each actual dose.

### Schema
Two new models (`prisma/schema.prisma`, migration `20260902_medication_administration_mar` — **not yet applied**, flag for Alex's manual Supabase MCP run):
- `MedicationScheduleTime` — one row per fixed daily clock time a medication is due (`timeOfDay` "HH:MM", Eastern wall-clock). Zero rows = PRN/as-needed. Independent of the existing free-text `frequency` label.
- `MedicationAdministration` — the actual per-dose log. A row only exists once something happened (`status`: given/refused/omitted, or a PRN dose) — a still-pending slot is computed at read time, never pre-materialized (no cron, unlike `ShiftTemplate`→`Shift`). `@@unique([medicationId, scheduledDate, scheduledTimeOfDay])` governs one-row-per-scheduled-slot; Postgres treats NULL as distinct so PRN rows (`scheduledTimeOfDay: null`) are naturally exempt and several can exist per day.

**Dual-actor pattern** (new to this codebase) — every row tracks two identities, both snapshotted at write time (`*DisplayNameSnapshot`, same "frozen, survives later name changes" convention as `ProgressNote.authorDisplayNameSnapshot`):
- `administeredBy*` — who actually gave the dose. Could be a guardian.
- `documentedBy*` — who is entering the record right now. Almost always the same person, except when a nurse is proxy-documenting a dose a family member actually gave (families often don't fill out their own paperwork — the point of this feature).

### Permissions (`lib/permissions.ts`)
`canViewMedicationAdministration` (whole linked care team, like `canViewProgressNote`). `canDocumentMedicationAdministration` — self-attesting is always allowed for any linked role; attributing to *someone else* is nurse/admin only, and the target is verified server-side as an actual nurse-or-guardian linked to the patient (never just trusted from the request body). `canEditMedicationAdministration` — whoever documented the entry, or admin.

### Schedule-time configuration
`MedicationList.tsx`'s form gained a "Scheduled Times" section (hidden for PRN frequency) — `scheduleTimes: string[]` threaded through all three role-scoped medication CRUD routes (`nurse/patients/[id]/medications`, `family/medications`, `admin/patients/[id]/medications`) via shared helpers in `lib/medicationScheduleTimes.ts` (`scheduleTimesCreateData` for create, `replaceScheduleTimes` for a PATCH's delete-and-recreate — same pattern `ProgressNote`'s vitals/IO PATCH already uses). `lib/pharmacyLookup.ts`'s `flattenMedication` sorts and flattens the relation into a plain string array for the client.

### Role-agnostic API — `app/api/patient/[id]/medication-administrations/`
Matches the newer role-agnostic convention (`/api/patient/[id]/{shifts,appointments,calendar}`), not the older per-role medication-CRUD routes.
- `GET ?start=&end=` (YYYY-MM-DD date keys, defaults to the current week) — computes the full grid: for each active medication, expands `MedicationScheduleTime` × each day into expected slots, left-joined against existing rows; PRN meds return their entries grouped by day instead.
- `POST` — mark a dose. Upserts on `[medicationId, scheduledDate, scheduledTimeOfDay]` for a scheduled slot; always creates new for PRN. Takes `administeredTimeOfDay` (Eastern "HH:MM", optional — omitted means "right now") rather than a client-constructed instant, resolved server-side via `easternTimeOfDayUtc` per the Time/date-of-day convention — never trusts the browser's own timezone.
- `[entryId]/route.ts` `PATCH`/`DELETE` — correct or remove one entry, gated by `canEditMedicationAdministration`.
- `lib/medicationAdministrationActor.ts` — shared `sessionDisplayName`/`resolveAdministeredByActor` snapshot-resolution helpers used by both routes.

### UI
`app/components/patient/PatientMedicationMAR.tsx` — new "Medication Log" tab (`PATIENT_DETAIL_TABS`'s `medicationLog` key) on all three patient-detail pages. Dense grid (medications/scheduled-times down the side, days across the top), Week default with a Month toggle (reuses `lib/calendarViewRange.ts`'s `computeViewRange`/`shiftAnchor`), kept a real table on mobile too — horizontal scroll + fade hint, same pattern as `QuickNoteForm.tsx`'s Vitals/IO tables (List vs. card UI convention). Clicking any cell opens a modal (status, administered-by picker, time, reason, notes); when `administeredByUserId` differs from `documentedByUserId` the modal shows both distinctly. Admin and nurse get the attribution dropdown (`canAttributeToOthers`, roster built from `nurseLinks`/`guardianLinks` — required adding `userId` to the `nurse` select in the nurse/admin single-patient GET routes and to `PatientCareTeam.tsx`'s `NurseLink` type, since the existing `nurse.id` there is `NurseProfile.id`, not `User.id`); family/guardian is self-only.
