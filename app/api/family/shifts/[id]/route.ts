...
import { canEditShift, canCancelShift } from '../../../../../lib/permissions'
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (data.nurseId) {
    const link = await prisma.nursePatient.findUnique({
      where: { nurseId_patientId: { nurseId: data.nurseId, patientId: shift.patientId } },
    })
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const shift = await prisma.shift.findUnique({ where: { id } })
  if (!shift) return NextResponse.json({ error: 'Not found' }, { status: 404 })
  if (!(await canCancelShift(session, shift))) return NextResponse.json({ error: 'Not found' }, { status: 404 })
