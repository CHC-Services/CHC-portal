...
- An explicit clock time is stated (e.g. "at 6:15am", "at 7") — use that exact stated time, converted to 4-digit 24-hour military format, directly as the prefix. Do not run it through any further arithmetic (don't add it to a duration, don't offset it from a nearby time) — an explicitly stated time is already the answer, not an input to compute from.
- No explicit clock time was given for this next action, but a duration was given for the activity just described (e.g. "for 10 minutes", "lasted 10 min") and the entry then moves on to a new, distinct action — only then, add that duration to the running clock to get that next action's start time. Only advance the clock this way when a new distinct action actually follows with no explicit time of its own; a duration describing the entry's only activity doesn't need a second line, and an explicitly stated time (the first bullet above) always wins over computing one this way.

Each extracted or computed time becomes the timestamp prefix for its own narrative line — this takes priority over the entry's own recorded-at time, which only applies when nothing in the entry's own text lets you resolve a more specific time — and determines that line's chronological position among every other line in the narrative, same one-blank-line-between-entries formatting as everywhere else. This still applies no matter how many entries or how many extracted times are in the shift — a long, dense shift with a dozen-plus timestamped actions gets exactly the same per-line treatment as a short one; don't fall back to an entry's recorded-at time just because there are many entries to get through.

Once a time or duration phrase has been extracted and used as a line's prefix, remove that phrase from the sentence body — the prefix already carries it, so "at 6:15am"/"at 0615"/"10 minutes later" (or any equivalent) must not also appear inside the sentence text repeating the same value. A narrative line's prefix and the time-of-day phrasing inside its own sentence must never disagree — if you catch yourself about to write a body sentence that states a different clock time than the prefix you just gave that same line, the extraction was done wrong; re-resolve which time actually belongs to that line's own action instead of emitting the mismatch.
...
- Once a value has been pulled into its own vitals row, do not also restate it as a standalone vital-signs sentence in the narrative (e.g. don't additionally write a narrative line like "Temp 98.6, HR 72, RR 18" once those same numbers are sitting in a vitals row for that timestamp) — the vitals table is where that value lives now, and repeating it in prose is redundant. This only trims a bare recitation of the vital-sign values themselves; any other clinical observation, action, or context dictated in the same breath (e.g. "lungs clear, patient resting comfortably," "repositioned patient after taking vitals") still belongs in the narrative as normal.
...
- O2 percent (o2Percent): a pre-ductal/post-ductal pair dictated together as one measurement (e.g. "98 over 93", "preductal 98 and postductal 93") goes in the single o2Percent field as "98/93" — pre-ductal first, then post-ductal, separated by a slash, since both numbers were taken at the same time. If only one saturation number is dictated with no pre/post distinction, that single value IS the post-ductal reading — record it alone (e.g. "93"), never paired with a "-" or a slash.
- Medications are never their own intake row. When a medication is dictated — by name, or just described as "medications pushed via GT" — that content stays in the narrative only; do not create an intake row for the medication itself, nor for any solid food or texture-modifier (e.g. "thick pudding") mentioned only as the vehicle for administering it. Exception: if a tablet was dissolved or diluted in water, or flushed with water, before/after administration, that water gets its own ordinary Intake row like any other water intake — it's the fluid being tracked, not the medication.
