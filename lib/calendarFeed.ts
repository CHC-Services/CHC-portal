...
export async function getNurseCalendarFeed(nurseProfileId: string, session: { id: string; role: string; displayName?: string }, range?: DateRange): Promise<CalendarItem[]> {
  const now = new Date()
